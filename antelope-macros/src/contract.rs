...
pub fn add_contract_trait_impl(attrs: KVPairs, contract_struct: ItemStruct) -> TokenStream {
    ...
    quote! {
        ...
        #[doc(hidden)]
        const _: () = {
            impl antelope::Contract for #struct_name {
                fn account() -> antelope::AccountName {
                    const { antelope::AccountName::constant(#account) }
                }
                fn name() -> antelope::ActionName {
                    const { antelope:: ActionName::constant(#name) }
                }
            }
        };
    }
