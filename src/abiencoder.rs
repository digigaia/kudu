...
use crate::abi::*;
use crate::types::{
    ABISerializable, AntelopeType,
    Name, InvalidValue
};
use super::*;

pub struct ABIEncoder {
    // data stream where we serialize objects
    // FIXME: this should be passed as arguments to function that serialize, not be a member here
    //        this should also be made generic using the std::io::Write trait
    // data: ByteStream,
...
impl ABIEncoder {
    ...
    pub fn encode(&self, ds: &mut ByteStream, object: &impl ABISerializable) {
        object.encode(ds)
    }
    ...
    pub fn encode_variant(&self, ds: &mut ByteStream, typename: &str, object: &Value) -> Result<(), InvalidValue> {
        ...
        // if let Some(pf) = self.builtin_types.get(&ftype) {
        if AntelopeType::VARIANTS.contains(&&ftype[..]) {
            // if our fundamental type is a builtin type, use the associated packing function
            if is_array(rtype) {
            ...
            if is_array(rtype) {
                ...
                match !object.is_null() {
                    true => { AntelopeType::Bool(true).to_bin(ds); self.encode_variant(ds, &ftype, object)?; },
                    false => AntelopeType::Bool(false).to_bin(ds),
                }
            }
                ...
                if object.is_object() {
                    if !struct_def.base.is_empty() {
                        self.encode_variant(ds, self.resolve_type(&struct_def.base), object)?;
                    }
...
mod tests {
    ...
    use serde_json::json;
    use super::*;
    use super::super::types::{Name, Symbol, Asset};
